...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput("optical_collimation_date", "Optical Collimation Test Date (YYYY-MM-DD)"),
      textInput("optical_collimation", "Optical Collimation (4 decimal places)"),
...
server <- function(input, output) {
  ...
  observeEvent(input$update_info, {
    ...
    info_data <- data.frame(
      Variable = c("Party Name", "Site Number", "Date", "Start Time", "End Time", 
                  ...
                  "Optical Collimation Test Date", "Optical Collimation"),
      Value = c(input$party_name, input$site_number, input$date, input$time, input$end_time,
               ...
               input$optical_collimation_date, input$optical_collimation),
  ...
  output$download_xml <- downloadHandler(
    ...
    content = function(file) {
      ...
      xml2::xml_add_child(engineer_level, "OpticalCollimationTestDate") %>%
        xml2::xml_set_text(input$optical_collimation_date)
      xml2::xml_add_child(engineer_level, "OpticalCollimation") %>%
        xml2::xml_set_text(ifelse(is.na(as.numeric(input$optical_collimation)), "", 
                                format(as.numeric(input$optical_collimation), scientific = FALSE, nsmall=4)))
      ...
      # Add circuit summary - FIXED: Only includes ObjectName and Description
      ...
      for (i in 1:nrow(summary_data)) {
        ...
        object_name <- if(object_name == base_object) "BASE" else object_name
        xml2::xml_add_child(object, "ObjectName") %>%
          xml2::xml_set_text(object_name)
        # Change this to create tags with space
        description_node <- xml2::xml_add_child(object, "Description")
        # Instead of immediately setting text to empty string, we'll create the node first
        # xml2::xml_set_text(description_node, "")
      }
      comment_node <- xml2::xml_add_child(circuit_summary, "Comment")
      # No need to set empty text for Comment either
      
      # Add final summary - Keep as original
      ...
      for (i in 1:nrow(summary_data)) {
        ...
        xml2::xml_add_child(object, "ElevationFound") %>%
          xml2::xml_set_text(ifelse(is.na(summary_data$`Average Elevation`[i]), "", as.character(summary_data$`Average Elevation`[i])))
